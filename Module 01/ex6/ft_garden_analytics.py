...
class GardenManager:
    ...
    def validate_height(plants: list) -> bool:
        """Méthode statique utilitaire qui calcule la taille"""
        for pl in plants:
            if pl.height <= 0:
                return False
        return True
    ...
    def affiche_stat_garden(self) -> None:
        ...
        print("\n")
        value = GardenManager.validate_height(self.garden)
        print(f"Height validation test: {value}")
        sc_a = GardenManager.GardenStats.calcul_stat(self.garden)
